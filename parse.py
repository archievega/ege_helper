...
subjects_id = [1,2,3,4,5,6,7,8,10,11,12,13,18,22]
#subjects_id = [7]
old_dir = os.getcwd()
...
for i in subjects_id:
    ...
    response = requests.post('http://os.fipi.ru/api/tasks', headers=headers,
    ...
    for task in tasks:
        if task['taskTypeId'] == 1:
            text = " ".join(list(map(lambda x: x.text, BeautifulSoup(
                task['taskTextWord'], 'lxml').findAll("p"))))
            text = re.sub('\\r\\n\\\\*', ' ', text).strip()
            data = {
                "id": task["id"],
            ...
            answers = list(map(lambda x: x.text.strip(),
                           parsed_html.findAll("div", {"class": "answer"})))
            text = " ".join(list(map(lambda x: x.text,
                       BeautifulSoup(task['taskTextWord'], 'lxml').findAll("p"))))
            text = re.sub('\\r\\n\\\\*', ' ', text).strip()
            data = {
                "id": task["id"],
                "taskTypeId": task["taskTypeId"],
            ...
            data = {
                "id": task["id"],
                "taskTypeId": task["taskTypeId"],
                "answer_index": task["answer"].split(","),
                "answers": answers,
                "text": text,
            }
        else:
            continue
        themeNameId = task['themeNames']
        themeNameId = themeNameId[0].split()[0] if themeNameId else "0.0"
